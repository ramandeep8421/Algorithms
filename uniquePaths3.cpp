...
private:
    ...
    void dfs(int i, int j, int x, int y, vector<vector<int>>& grid, vector<vector<bool>>& visited) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            //If the new cell is already visited or its not an empty cell then continue to another cell
            if (newr < 0 || newr >= n || newc < 0 || newc >= m || grid[newr][newc] == 1 || visited[newr][newc]) {
                continue;
            }

            //move to the new cell
            dfs(newr, newc, x, y, grid, visited);
        }
